...
def find_virtual_cable_input_device():
    ...
    for i in range(0, num_devices):
        if (p.get_device_info_by_host_api_device_index(0, i).get('maxInputChannels')) > 0:
            ...
            print(f"Checking device: {device_name} (ID: {i})")
            # Look for the Voicemeeter virtual output device
            if "Voicemeeter Output" in device_name: # This is the virtual microphone for our app
                print(f"Found Voicemeeter Output device: {device_name} (ID: {i})")
                p.terminate()
                return i
            # Keep VB-Audio Virtual Cable as a fallback if you intend to use it, or remove it.
            # if "CABLE Output (VB-Audio Virtual Cable)" in device_name:
            #     print(f"Found VB-Audio Virtual Cable Output device: {device_name} (ID: {i})")
            #     p.terminate()
            #     return i
            # if "Stereo Mix" in device_name and sys.platform == "win32": # Common fallback
            #     print(f"Found Stereo Mix input device (fallback): {device_name} (ID: {i})")
            #     p.terminate()
            #     return i
    p.terminate()
    print("WARNING: Desired audio input device (Voicemeeter Output) not found.")
    print("Please ensure Voicemeeter is running and configured correctly.")
    print("Or, modify find_virtual_cable_input_device() to match your setup.")
    return -1
